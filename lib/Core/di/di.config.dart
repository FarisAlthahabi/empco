// GENERATED CODE - DO NOT MODIFY BY HAND

// **************************************************************************
// InjectableConfigGenerator
// **************************************************************************

// ignore_for_file: type=lint
// coverage:ignore-file

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:empco/Core/bloc/applications_cubit/cubit/applications_cubit.dart'
    as _i353;
import 'package:empco/Core/bloc/edit_profile/cubit/edit_profile_cubit.dart'
    as _i999;
import 'package:empco/Core/bloc/following_cubit/cubit/following_cubit.dart'
    as _i752;
import 'package:empco/Core/bloc/licence_cubit/cubit/licence_cubit.dart'
    as _i725;
import 'package:empco/Core/bloc/permissions_cubit/permissions_cubit.dart'
    as _i974;
import 'package:empco/Core/bloc/profile/cubit/profile_cubit.dart' as _i112;
import 'package:empco/Core/di/modules/app_modules.dart' as _i723;
import 'package:empco/Core/dio/dio_client.dart' as _i401;
import 'package:empco/Core/Localization/cubit/localization_cubit.dart' as _i827;
import 'package:empco/Core/Localization/language_changer.dart' as _i980;
import 'package:empco/Core/repos/applications/applications_repo.dart' as _i448;
import 'package:empco/Core/repos/edit_profile/edit_profile_repo.dart' as _i704;
import 'package:empco/Core/repos/following_repo/following_repo.dart' as _i526;
import 'package:empco/Core/repos/licence/licence_repo.dart' as _i688;
import 'package:empco/Core/repos/profile/profile_repo.dart' as _i1011;
import 'package:empco/Core/repos/user_repo/user_repo.dart' as _i592;
import 'package:empco/Features/Auth/cubit/auth_cubit.dart' as _i657;
import 'package:empco/Features/Auth/repo/auth_repo.dart' as _i788;
import 'package:empco/Features/auth_manager/bloc/auth_manager_bloc.dart'
    as _i393;
import 'package:empco/Features/Roles/Company/jop_post/cubit/job_post_cubit.dart'
    as _i705;
import 'package:empco/Features/Roles/Company/jop_post/service/job_post_service.dart'
    as _i592;
import 'package:empco/Features/Roles/Freelancer/apply_job/cubit/apply_job_cubit.dart'
    as _i801;
import 'package:empco/Features/Roles/Freelancer/apply_job/repo/apply_job_repo.dart'
    as _i611;
import 'package:empco/Features/Roles/Freelancer/Job_details/cubit/job_details_cubit.dart'
    as _i727;
import 'package:empco/Features/Roles/Freelancer/Job_details/Service/job_details_service.dart'
    as _i976;
import 'package:empco/Features/Roles/Freelancer/Jobs/cubit/jobs_cubit.dart'
    as _i457;
import 'package:empco/Features/Roles/Freelancer/Jobs/Service/jobs_service.dart'
    as _i638;
import 'package:empco/Features/Roles/Freelancer/service_your_emloyee/cubit/service_cubit.dart'
    as _i653;
import 'package:empco/Features/Roles/Freelancer/service_your_emloyee/repo/service_repo.dart'
    as _i635;
import 'package:get_it/get_it.dart' as _i174;
import 'package:injectable/injectable.dart' as _i526;
import 'package:shared_preferences/shared_preferences.dart' as _i460;

extension GetItInjectableX on _i174.GetIt {
// initializes the registration of main-scope dependencies inside of GetIt
  Future<_i174.GetIt> init({
    String? environment,
    _i526.EnvironmentFilter? environmentFilter,
  }) async {
    final gh = _i526.GetItHelper(
      this,
      environment,
      environmentFilter,
    );
    final registerModule = _$RegisterModule();
    gh.factory<_i974.PermissionsCubit>(() => _i974.PermissionsCubit());
    await gh.factoryAsync<_i460.SharedPreferences>(
      () => registerModule.prefs,
      preResolve: true,
    );
    gh.factory<_i827.LocalizationCubit>(() => _i827.LocalizationCubit());
    gh.singleton<_i401.DioClient>(() => _i401.DioClient());
    await gh.singletonAsync<_i980.LanguageChanger>(
      () {
        final i = _i980.LanguageChanger();
        return i.init().then((_) => i);
      },
      preResolve: true,
    );
    await gh.singletonAsync<_i592.UserRepo>(
      () {
        final i = _i592.UserRepo();
        return i.init().then((_) => i);
      },
      preResolve: true,
    );
    gh.factory<_i688.LicenceRepo>(() => _i688.HttpLicenceRepo());
    gh.factory<_i638.JobsService>(() => _i638.HttpJobService());
    gh.factory<_i976.JobDetailsService>(() => _i976.HttpJobDetailsService());
    gh.factory<_i611.ApplyJobRepo>(() => _i611.HttpApplyJobRepo());
    gh.factory<_i725.LicenceCubit>(
        () => _i725.LicenceCubit(gh<_i688.LicenceRepo>()));
    gh.factory<_i788.AuthRepo>(() => _i788.HttpAuthRepo());
    gh.factory<_i448.ApplicationsRepo>(() => _i448.HttpApplicationsRepo());
    gh.factory<_i526.FollowingRepo>(() => _i526.HttpFollowingRepo());
    gh.factory<_i801.ApplyJobCubit>(
        () => _i801.ApplyJobCubit(gh<_i611.ApplyJobRepo>()));
    gh.factory<_i1011.ProfileRepo>(() => _i1011.HttpProfileRepo());
    gh.factory<_i592.JobPostService>(() => _i592.HttpJobPostService());
    gh.factory<_i705.JobPostCubit>(
        () => _i705.JobPostCubit(gh<_i592.JobPostService>()));
    gh.factory<_i635.ServiceRepo>(() => _i635.HttpServiceRepo());
    gh.factory<_i704.EditProfileRepo>(() => _i704.HttpEditProfileRepo());
    gh.factory<_i999.EditProfileCubit>(
        () => _i999.EditProfileCubit(gh<_i704.EditProfileRepo>()));
    gh.factory<_i457.JobsCubit>(() => _i457.JobsCubit(gh<_i638.JobsService>()));
    gh.singleton<_i393.AuthenticationBloc>(
        () => _i393.AuthenticationBloc(gh<_i592.UserRepo>()));
    gh.factory<_i353.ApplicationsCubit>(
        () => _i353.ApplicationsCubit(gh<_i448.ApplicationsRepo>()));
    gh.factory<_i727.JobDetailsCubit>(
        () => _i727.JobDetailsCubit(gh<_i976.JobDetailsService>()));
    gh.factory<_i112.ProfileCubit>(
        () => _i112.ProfileCubit(gh<_i1011.ProfileRepo>()));
    gh.factory<_i657.AuthCubit>(() => _i657.AuthCubit(
          gh<_i788.AuthRepo>(),
          gh<_i592.UserRepo>(),
        ));
    gh.factory<_i653.ServiceCubit>(
        () => _i653.ServiceCubit(gh<_i635.ServiceRepo>()));
    gh.factory<_i752.FollowingCubit>(
        () => _i752.FollowingCubit(gh<_i526.FollowingRepo>()));
    return this;
  }
}

class _$RegisterModule extends _i723.RegisterModule {}
